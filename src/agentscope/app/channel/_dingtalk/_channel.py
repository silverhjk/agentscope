...
class DingTalkChannel(ChannelBase):
    ...
    async def send_notice(
        ...
        """Send a short Markdown notice into the event's encoded chat.

        Args:
            event (`ChannelEvent`): Target chat (``group:`` / ``user:`` id).
        ...
        if not event.chat_id:
            return False
        return await self.send_message_to(event.chat_id, text)
